...
export const Route = createFileRoute("/times")({
  head: () => ({
    meta: [
      { title: "Τιμές — SkydreamLabs" },
      { name: "description", content: "Διαφανείς τιμές για ταξιδιωτικά γραφεία κάθε μεγέθους. Χωρίς κρυφά κόστη, χωρίς per-user χρέωση που εκτροχιάζεται." },
      { property: "og:title", content: "Τιμές — SkydreamLabs" },
      { property: "og:description", content: "Τιμοκατάλογος SkydreamLabs για boutique ταξιδιωτικά γραφεία." },
      { property: "og:url", content: "/times" },
    ],
    links: [{ rel: "canonical", href: "/times" }],
  }),
  component: Pricing,
});
    ...
    <div>
      <section className="container-editorial pt-20 pb-14 md:pt-28 md:pb-14">
        ...
        <p className="mt-6 max-w-2xl text-lg text-ink-soft">
          Χωρίς per-user χρεώσεις που εκτροχιάζονται, χωρίς setup fees. Όλα τα πλάνα περιλαμβάνουν white-glove onboarding
          και ελληνόφωνη υποστήριξη.
        </p>
      </section>
        ...
        {plans.map((p) => (
          ...
          >
            <div className="flex items-baseline justify-between">
              <h3 className={`font-display text-2xl ${p.highlighted ? "text-paper" : "text-ink"}`}>{p.name}</h3>
              {p.highlighted && <span className="rounded-full bg-coral px-3 py-1 text-[10px] font-mono uppercase tracking-widest text-paper">Popular</span>}
            </div>
            <p className={`mt-1 text-sm ${p.highlighted ? "text-paper/70" : "text-ink-soft"}`}>{p.for}</p>
            <div className="mt-6 flex items-end gap-2">
              <span className={`font-display text-5xl ${p.highlighted ? "text-paper" : "text-ink"}`}>{p.price}</span>
              {p.tag && <span className={`pb-2 text-sm ${p.highlighted ? "text-paper/60" : "text-ink-soft"}`}>{p.tag}</span>}
            </div>
            <ul className="mt-8 space-y-3 text-sm flex-1">
              {p.features.map((f) => (
                <li key={f} className="flex gap-3">
                  <Check className={`h-4 w-4 flex-shrink-0 mt-0.5 ${p.highlighted ? "text-coral" : "text-coral"}`} />
