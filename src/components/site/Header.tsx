...
export function Header() {
  ...
  const links = [
    { to: "/platforma", label: t("nav.platform") },
    { to: "/pos-leitourgei", label: t("nav.how") },
    { to: "/gia-poion", label: t("nav.for") },
    { to: "/times", label: t("nav.pricing") },
    ...
    <header className="sticky top-0 z-40 border-b border-border/70 bg-paper/85 backdrop-blur-md">
      <div className="container-editorial flex h-16 items-center justify-between gap-6">
        <Link to="/" className="flex items-center gap-2 group">
          <span aria-hidden className="grid h-8 w-8 place-items-center rounded-full bg-sea-deep text-paper font-display text-lg leading-none">
            s
          </span>
          ...
          <nav className="container-editorial py-4 flex flex-col gap-3">
            {links.map((l) => (
              <Link
                ...
                className="py-1 text-ink-soft hover:text-coral"
                activeProps={{ className: "text-ink font-medium" }}
              >
                {l.label}
              </Link>
